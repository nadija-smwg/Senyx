...
export default function EmployeesPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setError(err?.message || "Could not load employees. Please try again.")
    } finally {
      setLoading(false)
    }
  }, [])
        ...
        <div className="relative px-6 py-5 md:px-7 md:py-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            <Button
              variant="outline"
              size="sm"
              className="gap-2 border-gray-200 bg-white hover:bg-gray-50"
              onClick={fetchData}
              disabled={loading}
            >
              {loading ? <Spinner className="h-3.5 w-3.5" /> : <RefreshCw className="h-4 w-4" />}
              Refresh
            </Button>
      ...
      <div className="rounded-xl bg-white border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)] overflow-hidden">
        ...
        {loading && data.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center text-center py-16 px-6">
            <div className="h-12 w-12 rounded-full bg-rose-50 flex items-center justify-center mb-3 ring-1 ring-rose-100">
              <AlertCircle className="h-6 w-6 text-rose-600" />
            </div>
            <h3 className="font-semibold text-slate-900">Couldn’t load employees</h3>
            <p className="text-sm text-slate-500 mt-1 max-w-md">{error}</p>
            <Button size="sm" className="mt-4 gap-2" onClick={fetchData}>
