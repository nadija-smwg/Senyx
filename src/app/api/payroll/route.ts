...
import { payrollRecords } from '../../../server/db/schema/hr';
...
export async function GET(req: NextRequest) {
  try {
    ...
    let records;
    if (isAdminOrHR) {
      records = await db.select().from(payrollRecords);
    } else {
      if (!ctx.employeeId) throw new UnauthorizedError('No employee profile associated');
      records = await db.select().from(payrollRecords).where(eq(payrollRecords.employeeId, ctx.employeeId));
    }
