    ...
    <div className="bg-white border border-gray-100 rounded-xl p-3 shadow-lg text-xs">
      ...
      {payload.map((e: any, i: number) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <span className="text-gray-900 font-semibold">{Number(e.value || 0).toLocaleString()}</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="p-4" style={{ height: height - 60 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 10, left: -10, bottom: 10 }}>
            ...
            <XAxis dataKey={xAxisKey} tick={{ fontSize: 11, fill: '#9CA3AF' }} axisLine={false} tickLine={false} dy={8} />
