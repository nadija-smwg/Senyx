    ...
    <div className="bg-white border border-gray-100 rounded-xl p-3 shadow-lg text-xs">
      ...
      {payload.map((entry: any, i: number) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <span className="text-gray-900 font-semibold tabular-nums">${Number(entry.value || 0).toLocaleString()}</span>
        </div>
      ))}
    </div>
  );
...
export function AreaChartWidget({ title, data, xAxisKey, areas }: AreaChartWidgetProps) {
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)] overflow-hidden h-full flex flex-col">
        ...
        <div className="h-[280px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
              ...
              <XAxis dataKey={xAxisKey} axisLine={false} tickLine={false} tick={{ fill: '#9CA3AF', fontSize: 11 }} dy={6} />
